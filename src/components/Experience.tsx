      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative border-l-2 border-indigo-200 dark:border-indigo-800 pl-6 ml-6 space-y-10">
          {experiences.map((exp, index) => (
            <div 
              key={index} 
              className="relative bg-white dark:bg-slate-800 rounded-xl shadow-md p-6"
            >
              {/* Timeline dot */}
              <div className="absolute w-4 h-4 bg-indigo-600 dark:bg-indigo-400 rounded-full -left-[30px] top-6 border-4 border-white dark:border-slate-900"></div>
              
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white">{exp.title}</h3>
                  <p className="text-indigo-600 dark:text-indigo-400 font-medium flex gap-2">{exp.company}{exp.companyIcon}</p>
                </div>
